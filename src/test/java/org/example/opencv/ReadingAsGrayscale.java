...
public class ReadingAsGrayscale extends Application {
    ...
    public WritableImage loadAndConvert() throws Exception {
        ...
        String input = "/Users/chmy/Pictures/shanghai_3.jpg";

        // Reading the image
        Mat src = Imgcodecs.imread(input, Imgcodecs.IMREAD_GRAYSCALE);

        byte[] data1 = new byte[src.rows() * src.cols() * (int) (src.elemSize())];
        src.get(0, 0, data1);

        // Creating the buffered image
        BufferedImage bufImage = new BufferedImage(src.cols(), src.rows(), BufferedImage.TYPE_BYTE_GRAY);
